...
##  Function
##-----------------------------------------------------------------------------
def matching(template_extr, mask_extr, temp_dir, threshold_list=[0.38]):
	"""
	Description:
	...
	Output:
		Dictionary with the result for each threshold, or -1 if the database is empty.
		Result for each threshold is a list, either empty of there were no matches,
		or containing strings of the matched files (templates).
	"""
	...
	# Use all cores to calculate Hamming distances between query image and all templates
	args = zip(
	...
	# with Pool(processes=6) as pools:
	...
	# Give results for all thresholds
	results = {}
	for threshold in threshold_list:
		...
		if len(ind_thres) == 0:
			...
			results[threshold] = [filenames[idx] for idx in ind_sort]
